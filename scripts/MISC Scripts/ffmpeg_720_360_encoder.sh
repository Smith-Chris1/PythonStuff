...
if [[ $filename == *"_MATCH_SOURCE"* ]]
	...
	#echo $bitRaten
	ffmpeg -y -i "$directory/$filename.mp4" -c:v libx264 -b:v $bitRate -preset veryslow -level 4.0 -vf unsharp -vf scale=1280:720 -strict -2 -c:a copy "$directory"/"$newName".mp4
else	 
